...
// ── Run Steps Block (kernel traceability) ────────────────────
const STEP_LABELS: Record<string, { label: string; icon: React.ElementType }> = {
  intake: { label: "Intake & Contexto", icon: LayoutGrid },
  sociobehavioral: { label: "Análise Sociocomportamental", icon: Users },
  offer_analysis: { label: "Engenharia de Oferta", icon: Zap },
  performance_timing: { label: "Performance & Timing", icon: BarChart3 },
  synthesis: { label: "Síntese Estratégica", icon: Target },
};

function RunStepsBlock({ analysisId }: { analysisId: string }) {
  ...
  return (
    <div className="glass-card p-6">
      <h3 className="section-label mb-4 flex items-center gap-2">
        <LayoutGrid className="h-4 w-4" /> Pipeline de Análise
      </h3>
      <div className="flex items-center gap-1 text-xs text-muted-foreground mb-4">
        <span>Modelo: {run.model_fallback || run.model_primary}</span>
        {run.duration_ms && <span> · {(run.duration_ms / 1000).toFixed(1)}s</span>}
      </div>
      <div className="space-y-2">
        {steps.map((step) => {
          const meta = STEP_LABELS[step.step_kind];
          if (!meta) return null;
          const Icon = meta.icon;
          const isCompleted = step.status === "completed";
          const isFailed = step.status === "failed";
          return (
            <div key={step.id} className="flex items-center gap-3 p-2 rounded-lg bg-accent/20">
              <div className={`flex h-7 w-7 items-center justify-center rounded-md ${
                isCompleted ? "bg-success/20" : isFailed ? "bg-destructive/20" : "bg-muted"
              }`}>
              ...
              <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                isCompleted ? "bg-success/20 text-success" : isFailed ? "bg-destructive/20 text-destructive" : "bg-muted text-muted-foreground"
              }`}>
                {isCompleted ? "Concluído" : isFailed ? "Falhou" : "Pendente"}
              </span>
            </div>
          );
        })}
