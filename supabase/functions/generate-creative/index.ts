...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Create a professional marketing creative image for:

HEADLINE: ${brief.headline}
BODY: ${brief.body_copy}
CTA: ${brief.cta}
VISUAL DIRECTION: ${brief.visual_direction}
FORMAT: ${brief.format || "1080x1080"}

Style: Modern, clean, professional marketing material. Bold typography. 
The image should be ready to use as a social media ad or banner.
Include the headline text prominently in the image.
Use vibrant colors that match the brand energy.
Do NOT include any placeholder or lorem ipsum text.`;
    ...
    return new Response(
      JSON.stringify({ image: imageUrl, text: textContent }),
